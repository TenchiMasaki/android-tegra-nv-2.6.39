...
#		else
static int alc5624_suspend(struct snd_soc_codec *codec)
#		endif
{
#	else
...
static int alc5624_probe(struct snd_soc_codec *codec)
{
	...
	for (i = 2; i < ALC5624_VENDOR_ID2; i+=2) {
		snd_soc_write(codec,i,alc5624_reg_default[i]);
	}
	for (; i < ARRAY_SIZE(alc5624_reg_default); i++) {
		snd_soc_write(codec,i,alc5624_reg_default[i]);
	}
	...
	snd_soc_dapm_add_routes(dapm, alc5624_dapm_routes, ARRAY_SIZE(alc5624_dapm_routes));
...
static int alc5624_plat_probe(struct platform_device *pdev)
{
	...
	/* set up audio path interconnects */
	snd_soc_dapm_add_routes(alc5624_codec, alc5624_dapm_routes, ARRAY_SIZE(alc5624_dapm_routes));
